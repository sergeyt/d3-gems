...
(function() {
	...
	formatNumber = function( value, format, culture ) {
		...
		function padLeft(s, length) { while (s.length < length) s = '0' + s; return s; }
		function padRight(s, length) { while (s.length < length) s = s + '0'; return s; }

		format = format || "g";
		
		if (format.length == 1 || (format.length > 1 && format.substr(1).match(/^\d+$/))) {
			...
			switch (format.charAt(0)) {
				...
				case 'g':
					...
					if (precision < 0) precision = nf.decimals;
					return formatFixed();
				...
				case 'D':
					...
					if (negative && value < 0) {
						return nf["-"] + padLeft(s, precision - 1);
					}
					return padLeft(s, precision);
				...
				case 'P':
					if (precision < 0) precision = 0;
					value = value * 100;
					return formatFixed() + nf.percent.symbol;
					
				case "c":
				case "C":
					if (precision === -1) precision = nf.decimals;
					var pattern = value < 0 ? nf.currency.pattern[0] : (nf.currency.pattern[1] || "n");
					var number = expandNumber(Math.abs(value), precision, nf);
					return formatPattern(number, pattern);
			}
		}

		return formatNumberCustom(value, format, culture);

		function formatPattern(number, pattern) {
		...
		function formatNumberCustom(value, format, culture) {
			var sectionInfo = findFormatSection(value, format);
			format = sectionInfo.format;

			var positive = true;
			if (value < 0) {
				if (!sectionInfo.negative) positive = false;
				value = -value;
			}
			...
			if (format.length == 0) return positive ? "" : nf["-"];

			var info = parseCustomNumericFormat(format);
			...
			if (decPointPos < 0) decPointPos = -1;
			...
			if (info.UseExponent) {
				...
				partInt = padLeft(partInt, info.IntegerDigits);
				partExp = padLeft(partExp, info.ExponentDigits - info.ExponentTailSharpDigits);
				...
				partInt = padLeft(partInt, info.IntegerDigits - info.IntegerHeadSharpDigits);
				if (info.IntegerDigits == info.IntegerHeadSharpDigits && isZeroOnly(partInt))
					partInt = "";
			}

			partDec = zeroTrimEnd(partDec, true);
			partDec = padRight(partDec, info.DecimalDigits - info.DecimalTailSharpDigits);
			if (partDec.length > info.DecimalDigits)
				partDec = partDec.substr(0, info.DecimalDigits);

			return formatNumberCustomCore(info, format, nf, positive, partInt, partDec, partExp);
		}

		function formatNumberCustomCore(info, format, nf, positive, partInt, partDec, partExp) {
			...
			var groupSize = nf.groupSizes[0];
			if (info.UseGroup && partInt.length > groupSize) {
				partInt = insertSep(partInt, nf[","], groupSize);
			}
			...
			for (i = 0; i < length; i++) {
				...
				if (c == literal && c != '\0') { literal = '\0'; continue; }
				if (literal != '\0') { sb += c; continue; }

				switch (c) {
					case '\\':
						i++;
						if (i < length) sb += format.charAt(i);
						break;
					case "'":
					case '"':
						literal = c;
					...
					case 'e':
					case 'E':
						if (partExp == null || !info.UseExponent) {
							sb += c;
							break;
						}
					...
					case '.':
						...
						break;
					case ',': break;
					case '%': sb += nf.percent.symbol; break;
					case '\u2030': sb += perMilesSymbol; break;
					default: sb += c; break;
				}
			}

			if (!positive) sb = nf["-"] + sb;

			return sb;
		}

		function parseCustomNumericFormat(format) {
			...
			for (var i = 0; i < format.length; i++) {
				...
				switch (c) {
					...
					case '.':
						...
						break;
					case '%': info.Percents++; break;
					case '\u2030': info.Permilles++; break;
					case ',':
		...
		function findFormatSection(value, format) {
		...
		function zeroTrimEnd(s, canEmpty) {
			var count = 0;
			for (var i = s.length - 1; (canEmpty ? i >= 0 : i > 0) ; i--) {
				if (s.charAt(i) != '0') break;
				count++;
			}
			if (count > 0) s = s.substr(0, s.length - count);
			return s;
		}
